...
do
  ...
  if [ "${opt_elem}" = "off" -o "${opt_elem}" = "OFF" -o "${opt_elem}" = "--nodoc" ]
  then
...
do
  ...
  then
    echo ""
    echo "Usage:"
    echo "    \$0 [--prefix=<install_dir> | --with-stdair=<stdair_install_dir>] [--with-doc | --without-doc] [-n|-N|--norm]"
...
#
CMAKE_CMD="cmake \${PREFIX_OPTION} \${STDAIR_OPTION} \${LIB_OPTION} \${BUILD_OPTION} \${DOC_OPTION} .."
